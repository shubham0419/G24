...
export default function Home() {
  ...
  useEffect(()=>{
    const newSocket = io("https://g25-xffu.onrender.com");
    setSocket(newSocket);
  },[])

  const gettAllPosts = async ()=>{
    if(isLoggenIn){
      let res = await axios.get("https://g25-xffu.onrender.com/post/all");
  ...
  const handlePostCreate = async (e)=>{
    ...
    let res = await axios.post("https://g25-xffu.onrender.com/post/create",payload);
    if(res.status==201){
      setRefresh(prev=>prev+1);
    }
  }
  ...
  const handlePostLike = async (id)=>{
    let res = await axios.post(`https://g25-xffu.onrender.com/post/like/${id}/${username}`);
    if(res.status=200){
      setRefresh(prev=>prev+1);
    }
  }
